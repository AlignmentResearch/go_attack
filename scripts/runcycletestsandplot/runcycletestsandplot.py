...
def process_sgf_file(filename,func):
  with open(filename, "r") as f:
    ...
    moves_since_setup = []
    komi = game.get_komi()
    walk_game_tree(filename,game.get_root(),board.copy(),board.copy(),moves_since_setup,komi,func)

def walk_game_tree(filename,node,board_at_setup,board,moves_since_setup,komi,func):
  board = board.copy()
  ...
  if node.has_property("C"):
    if node.find_property("C").strip() == "START":
      ...
      target_winner = None
      for child in node:
        if child.has_property("C") and child.find_property("C").strip() == "CORRECT":
          childcolor, childraw = child.get_raw_move()
          childmove = sgfmill.sgf_properties.interpret_go_point(childraw, board.side)
          correct_moves.append((childcolor,childmove))
        if child.has_property("C") and child.find_property("C").strip() == "WRONG":
          childcolor, childraw = child.get_raw_move()
          childmove = sgfmill.sgf_properties.interpret_go_point(childraw, board.side)
          wrong_moves.append((childcolor,childmove))

      func(filename,board_at_setup,moves_since_setup,komi,correct_moves,wrong_moves,target_winner)
    elif node.find_property("C").strip() == "BLACKWIN":
      target_winner = "b"
      func(filename,board_at_setup,moves_since_setup,komi,[],[],target_winner)
    elif node.find_property("C").strip() == "WHITEWIN":
      target_winner = "w"
      func(filename,board_at_setup,moves_since_setup,komi,[],[],target_winner)

  for child in node:
    walk_game_tree(filename,child,board_at_setup,board,moves_since_setup,komi,func)
...
if __name__ == "__main__":
  ...
  for model_path in sorted(os.listdir(models_path)):
    models.append((os.path.join(models_path,model_path), model_path[len("kata1-b60c320-"):len("kata1-b60c320-")+5]))
  ...
  def process(filename,board_at_setup,moves_since_setup,komi,correct_moves,wrong_moves,target_winner):
    for katago in katagos:
      response = katago.query(board_at_setup, moves_since_setup, komi)
      player = "b" if len(moves_since_setup) == 0 or moves_since_setup[-1][0] == "w" or moves_since_setup[-1][0] == "W" else "w"
      ...
      if len(correct_moves) > 0:
        ...
        correct_policy_mass, correct_search_mass = get_policy_and_search_mass(board_at_setup, response, correct_moves)
      ...
      if len(wrong_moves) > 0:
        ...
        wrong_policy_mass, wrong_search_mass = get_policy_and_search_mass(board_at_setup, response, wrong_moves)
      ...
      response = katago.query(board_at_setup, moves_since_setup, komi, max_visits=1)
  ...
  def plot_policy(plotfilename, ylabel, series_names, data):
    data = {series_name : data[series_name] for series_name in sorted(data.keys()) if series_name in series_names}
    ...
    for (series_name, series_data), color in zip(data.items(), colors):
      ...
      ax.plot(model_names, policy_values, label=series_name, color=color, marker="o")
    ...
    ax.set_ylim([0, 1])
    ax.yaxis.set_major_formatter(plt.FuncFormatter(lambda y, _: "%.3g%%" % (y*100.0)))
  ...
  def plot_winrate(plotfilename, ylabel, series_names, data):
    ...
    fig, ax = plt.subplots(figsize=(12.5,8.2))
    for (series_name, series_data), color in zip(data.items(), colors):
      winrate_values = [ytoplot(series_data[model_name]) for model_name in model_names]
      ax.plot(model_names, winrate_values, label=series_name, color=color, marker="o")
    ...
    ax.set_yticks([ytoplot(tick[0]) for tick in yticks],[tick[1] for tick in yticks])
  ...
  race_series_names = [key for key in series_names if key.startswith("race") and "already" not in key]
  plot_policy(os.path.join(plots_path,"race-raw.png"), "Correct Raw Policy", race_series_names, correct_policy_masses)
  plot_policy(os.path.join(plots_path,"race-search.png"), "Correct Search Mass", race_series_names, correct_search_masses)
  plot_winrate(os.path.join(plots_path,"race-rawwinrate.png"), "Raw Winrate", race_series_names, raw_winrate)
  plot_winrate(os.path.join(plots_path,"race-searchwinrate.png"), "Search Winrate", race_series_names, search_winrate)
  ...
  racealready_series_names = [key for key in series_names if key.startswith("race") and "already" in key]
  plot_policy(os.path.join(plots_path,"racealready-raw.png"), "Non-Wrong Raw Policy", racealready_series_names, correct_policy_masses)
  plot_policy(os.path.join(plots_path,"racealready-search.png"), "Non-Wrong Search Mass", racealready_series_names, correct_search_masses)
  plot_winrate(os.path.join(plots_path,"racealready-rawwinrate.png"), "Raw Winrate", racealready_series_names, raw_winrate)
  plot_winrate(os.path.join(plots_path,"racealready-searchwinrate.png"), "Search Winrate", racealready_series_names, search_winrate)

  escape_series_names = [key for key in series_names if key.startswith("escape")]
  plot_policy(os.path.join(plots_path,"escape-raw.png"), "Correct Raw Policy", escape_series_names, correct_policy_masses)
  plot_policy(os.path.join(plots_path,"escape-search.png"), "Correct Search Mass", escape_series_names, correct_search_masses)
  plot_winrate(os.path.join(plots_path,"escape-rawwinrate.png"), "Raw Winrate", escape_series_names, raw_winrate)
  plot_winrate(os.path.join(plots_path,"escape-searchwinrate.png"), "Search Winrate", escape_series_names, search_winrate)
  ...
  distraction_series_names = [key for key in series_names if key.startswith("distraction")]
  plot_policy(os.path.join(plots_path,"distraction-raw.png"), "Correct Raw Policy", distraction_series_names, correct_policy_masses)
  plot_policy(os.path.join(plots_path,"distraction-search.png"), "Correct Search Mass", distraction_series_names, correct_search_masses)
  plot_winrate(os.path.join(plots_path,"distraction-rawwinrate.png"), "Raw Winrate", distraction_series_names, raw_winrate)
  plot_winrate(os.path.join(plots_path,"distraction-searchwinrate.png"), "Search Winrate", distraction_series_names, search_winrate)
  ...
  eyelive_series_names = [key for key in series_names if key.startswith("eye") and "live" in key]
  plot_policy(os.path.join(plots_path,"eyelive-raw.png"), "Correct Raw Policy", eyelive_series_names, correct_policy_masses)
  plot_policy(os.path.join(plots_path,"eyelive-search.png"), "Correct Search Mass", eyelive_series_names, correct_search_masses)
  plot_winrate(os.path.join(plots_path,"eyelive-rawwinrate.png"), "Raw Winrate", eyelive_series_names, raw_winrate)
  plot_winrate(os.path.join(plots_path,"eyelive-searchwinrate.png"), "Search Winrate", eyelive_series_names, search_winrate)

  eyekill_series_names = [key for key in series_names if key.startswith("eye") and "kill" in key]
  plot_policy(os.path.join(plots_path,"eyekill-raw.png"), "Correct Raw Policy", eyekill_series_names, correct_policy_masses)
  plot_policy(os.path.join(plots_path,"eyekill-search.png"), "Correct Search Mass", eyekill_series_names, correct_search_masses)
  plot_winrate(os.path.join(plots_path,"eyekill-rawwinrate.png"), "Raw Winrate", eyekill_series_names, raw_winrate)
  plot_winrate(os.path.join(plots_path,"eyekill-searchwinrate.png"), "Search Winrate", eyekill_series_names, search_winrate)

  inevitable_series_names = [key for key in series_names if key.startswith("inevitable")]
  plot_winrate(os.path.join(plots_path,"inevitable-rawwinrate.png"), "Raw Winrate", inevitable_series_names, raw_winrate)
  plot_winrate(os.path.join(plots_path,"inevitable-searchwinrate.png"), "Search Winrate", inevitable_series_names, search_winrate)

  statusdead_series_names = [key for key in series_names if key.startswith("statusdead")]
  plot_winrate(os.path.join(plots_path,"statusdead-rawwinrate.png"), "Raw Winrate", statusdead_series_names, raw_winrate)
  plot_winrate(os.path.join(plots_path,"statusdead-searchwinrate.png"), "Search Winrate", statusdead_series_names, search_winrate)

  statusalive_series_names = [key for key in series_names if key.startswith("statusalive")]
  plot_winrate(os.path.join(plots_path,"statusalive-rawwinrate.png"), "Raw Winrate", statusalive_series_names, raw_winrate)
  plot_winrate(os.path.join(plots_path,"statusalive-searchwinrate.png"), "Search Winrate", statusalive_series_names, search_winrate)
