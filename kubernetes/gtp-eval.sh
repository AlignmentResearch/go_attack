...
/gogui/bin/gogui-twogtp \
  -black "/engines/KataGo-raw/cpp/katago gtp -config $VICTIM_CONFIG -config $GPU_CONFIG -model $VICTIM_MODEL" \
  -white "/engines/KataGo-custom/cpp/katago gtp -config $ADV_CONFIG -config $GPU_CONFIG -model $ADV_MODEL -victim-model $VICTIM_MODEL" \
