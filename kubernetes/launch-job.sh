...
usage() {
  ...
  echo "    Only for and is required for iterated training. Path to the TF weights"
  echo "    directory for the last victim in the initial curriculum."
  echo "  -w, --use-weka"
...
if [ -n "${USE_ITERATED_TRAINING:-}" ]; then
  ...
  SHUFFLE_AND_EXPORT_CMD="bash -c \"/go_attack/kubernetes/shuffle-and-export.sh $RUN_NAME $RUN_NAME $VOLUME_NAME $USE_GATING && sleep infinity\""
