    ...
    if [[ "$LATEST_MODEL_DIR" =~ -s([0-9]+) ]]; then
        ...
        if [ "$STEP" -gt "$LAST_STEP" ]; then
            for VICTIM in "${victim_array[@]}"; do
                ...
                sed "s/0=/2=/" "$VICTIMS_DIR"/victim.cfg > /tmp/victim2.cfg
                sed "s/0=/3=/" "$VICTIMS_DIR"/victim.cfg > /tmp/victim3.cfg
                sed "s/0=/4=/" "$VICTIMS_DIR"/victim.cfg > /tmp/victim4.cfg
                ...
                $KATAGO_BIN match \
                    ...
                    -config /tmp/victim2.cfg \
                    -config /tmp/victim3.cfg \
                    -config /tmp/victim4.cfg \
                    ...
                    -override-config nnModelFile2="$VICTIMS_DIR"/"$VICTIM" \
                    -override-config botName2="victim-$VICTIM_NAME-revertopt" \
