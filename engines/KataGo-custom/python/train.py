...
def main(rank: int, world_size: int, args, multi_gpu_device_ids, readpipes, writepipes, barrier):
    ...
    def save(ddp_model, swa_model, optimizer, metrics_obj, running_metrics, train_state, last_val_metrics, model_config, path=None):
    ...
    while True:
        ...
        save(ddp_model, swa_model, optimizer, metrics_obj, running_metrics, train_state, last_val_metrics, model_config)
        ...
        if rank == 0:
            ...
            if not no_export and is_time_to_export and not skip_export_this_time and exportdir is not None and not gnorm_stats_debug:
                ...
                if os.path.exists(savepath):
                    ...
                    save(ddp_model, swa_model, optimizer, metrics_obj, running_metrics, train_state, last_val_metrics, model_config, path=os.path.join(savepathtmp,"model.ckpt"))
                    time.sleep(2)
        ...
        if rank == 0:
            ...
            if now - last_longterm_checkpoint_save_time >= datetime.timedelta(hours=12):
                ...
                save(ddp_model, swa_model, optimizer, metrics_obj, running_metrics, train_state, last_val_metrics, model_config, path=os.path.join(longterm_checkpoints_dir,f"{dated_name}.ckpt"))
