"""
This script adapts export_model.py to export a model which outputs
a constant policy/value net with the following configurable options:
  1. non-pass logit value
  2. pass logit value
  3. white-win logit value
  4. white-loss logit value
...
description = "Export neural net weights that encode a constant policy."
...
parser.add_argument('-no-pass-lv', type=float, default=0, help="non-pass logit value")
parser.add_argument('-pass-lv', type=float, default=2, help="pass logit value")
parser.add_argument('-win-lv', type=float, default=1, help="win logit value")
parser.add_argument('-loss-lv', type=float, default=0, help="loss logit value")
parser.add_argument('-nores-lv', type=float, default=-5000, help="no-result logit value")
...
no_pass_lv = args["no_pass_lv"]
pass_lv = args["pass_lv"]
win_lv = args["win_lv"]
loss_lv = args["loss_lv"]
nores_lv = args["nores_lv"]
...
with tf.compat.v1.Session(config=tfconfig) as session:
  ...
  if not for_cuda:
    ...
    def write_bn_constant(name: str, num_channels: int, c: float):
      """
      Modified version of write_bn that generates a constant output c.
      Formula:
        scale * (batch - moving_mean) / sqrt(moving_var + epsilon) + bias
      """
      writeln(name)
      (nc,epsilon,has_bias,has_scale,use_fixup) = model.batch_norms[name]
      assert(nc == num_channels)
      ...
      # moving_mean
      write_weights(np.zeros([num_channels]))
      ...
      # bias
      write_weights(c * np.ones([num_channels]))
    ...
    def write_policy_head():
      ...
      # write_bn("p1/norm",model.p1_num_channels) # OLD
      write_bn_constant("p1/norm", model.p1_num_channels, c=1.0) # NEW
      ...
      # write_conv(p2name+"/w",p2diam,p2in_channels,1,1,get_weights(p2name+"/w")[:,:,:,0:1]) # OLD
      # write_matmul("matmulpass",model.g2_num_channels,1,get_weights("matmulpass")[:,0:1]) # OLD
      write_conv(p2name+"/w",p2diam,p2in_channels,1,1,get_weights(p2name+"/w")[:,:,:,0:1] * 0 + no_pass_lv / p2in_channels) # NEW
      write_matmul("matmulpass",model.g2_num_channels,1,get_weights("matmulpass")[:,0:1] * 0 + pass_lv / model.g2_num_channels) # NEW
    ...
    def write_value_head():
      ...
      # write_matmul("v3/w",model.v2_size,model.v3_size,get_weights("v3/w")) # OLD
      write_matmul("v3/w",model.v2_size,model.v3_size,get_weights("v3/w") * 0) # NEW

      # OLD
      # if model.support_japanese_rules:
      #   write_matbias("v3/b",model.v3_size,get_weights("v3/b") * 0) # !!!MODIFIED!!!
      # else:
      #   w = get_weights("v3/b") * 0  # ZEROED OUT!
      #   assert(len(w.shape) == 1 and w.shape[0] == 3)
      #   w[2] = w[2] - 5000.0
      #   write_matbias("v3/b",model.v3_size,w)
      write_matbias("v3/b",model.v3_size, get_weights("v3/b") * 0 + np.array([win_lv, loss_lv, nores_lv])) # NEW
